...
const { data: post } = await useAsyncData(`photograph-${slug}`, () =>
  queryCollection("photograph").path(`/photograph/${slug}`).first(),
);
useHead({
  ...
  meta: [
    {
      name: "og:title",
      content: (post?.value?.title || "Photograph") + " | " + siteConfig.title,
    },
  ...
  <div class="w-full">
    ...
    <template v-else>
      <div class="empty-page p-4">
        <h1>ポストが見つかりませんでした</h1>
        <p>お探しのポストは存在しないか、移動しました。</p>
      </div>
    </template>
  </div>
</template>
